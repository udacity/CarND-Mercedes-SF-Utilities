...
acc_per_sec = 0.2; % acc in m/s^2 per sec
yaw_acc_per_sec = 0.2; % yaw acc in rad/s^2 per sec
...
while ischar(tline)  % go through lines of data file
    ...
    if stop_for_sigmavis && k == 25
        ...
        Xi_aug_p1 =  squeeze(Xi_aug(1,:,:));
        Xi_aug_p2 =  squeeze(Xi_aug(2,:,:));
        
        figure(4)
        hold on;
        plot(x_est(1,1), x_est(2,1), '-or');
        plot(Xi_aug_p1, Xi_aug_p2, 'xb');
        legend('est', 'Xi aug')
